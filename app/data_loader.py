        ...
        for _, row in exercises_df.iterrows():
            try:
                ...
                exercise = {
                    ...
                    'short_name': str(row['IBFexSHORT FORM NAME']) if not pd.isna(row['IBFexSHORT FORM NAME']) else None,
                    'aka': str(row['AKA']) if not pd.isna(row['AKA']) else None,
                    'phase_reviewer': str(row['Phase_reviewer']) if not pd.isna(row['Phase_reviewer']) else None
                }
        ...
        for _, row in musics_df.iterrows():
            try:
                ...
                music = {
                    ...
                    'collection_cd': str(row["Music 'CD' (Genre tag)"]) if not pd.isna(row["Music 'CD' (Genre tag)"]) else None,
                    'filename': str(row['Music filename']) if not pd.isna(row['Music filename']) else None,
                    'title': str(row['Music Title (Movement Name tag)']) if not pd.isna(row['Music Title (Movement Name tag)']) else None,
                    'artist': str(row['Music Artist (Artist tag)']) if not pd.isna(row['Music Artist (Artist tag)']) else None,
                    'duration': duration,
        ...
        for _, row in mappings_df.iterrows():
            try:
                ...
                mapping = {
                    ...
                    'music_ref': str(row['MusicRef']),
                    'recommendation': str(row['Recommendation']) if not pd.isna(row['Recommendation']) else None,
                    'specific_comment': str(row['Exercise-Music specific comment']) 
                                       if not pd.isna(row['Exercise-Music specific comment']) else None
                }
                mappings.append(mapping)
            except Exception as e:
                print(f"Warning: Error processing mapping for exercise {row['IBFex']} and music {row['MusicRef']}: {e}")
